    ...
    T: AsRef<[u8]> + 'static,
{
    ...
    let response = write_all(stream, request)
        .and_then(|(stream, _)| {
    ...
    let skip = response
        .and_then(move |(reader, buf)| {
            ...
            future::loop_fn((reader, buf), skip_headers).map(|reader| reader.into_inner())
            // FIXME: may lost data in buffer?
        })
        .map(|reader| reader.into_inner());
    if let Some(data) = data {
        future::Either::A(skip.and_then(|stream| write_all(stream, data).map(|(stream, _)| stream)))
    } else {
